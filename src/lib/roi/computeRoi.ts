...
export type RoiScenarioOverrides = {
  missedCallRateOverride?: number;
  avgJobValueOverride?: number;
};

function computeRoiInternal(
  trade: TradeKey,
  calls: number,
  scenarioIndex: ScenarioIndex,
  overrides: RoiScenarioOverrides = {},
): RoiResult {
  const baseTrade = TRADES[trade];
  const t = {
    ...baseTrade,
    missedCallRate: overrides.missedCallRateOverride ?? baseTrade.missedCallRate,
    avgJobValue: overrides.avgJobValueOverride ?? baseTrade.avgJobValue,
  };
  const s = scenarioIndex;
  ...
  const recoveredBookingRateEffective = t.baseBookingConv * SHARED.recoveredCallCaptureRatio[s];
  const jobsRecoveredPerMonth =
    calls * t.missedCallRate * recoveredBookingRateEffective;
  const missedCallRecoveryAnnual = roundWhole(
    jobsRecoveredPerMonth * t.avgJobValue * 12,
...
export function computeAllScenariosWithOverrides(
  trade: TradeKey,
  calls: number,
  overrides: RoiScenarioOverrides = {},
): RoiResult[] {
  const hasOverrides =
    overrides.missedCallRateOverride != null ||
    overrides.avgJobValueOverride != null;

  if (!hasOverrides) {
    return computeAllScenarios(trade, calls);
  }

  return ([0, 1, 2] as ScenarioIndex[]).map((scenarioIndex) =>
    computeRoiInternal(trade, calls, scenarioIndex, overrides),
  );
}
