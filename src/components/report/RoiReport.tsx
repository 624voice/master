  ...
  return (
    <section className="report-page">
      ...
      <div className="report-page-body">
        ...
        <p className="report-cta-preview">{PAGE1_CTA_PREVIEW}</p>
      </div>
...
function DriversPage({ model, logoSrc }: RoiReportProps) {
  return (
    <section className="report-page">
      <ReportHeader model={model} logoSrc={logoSrc} />
      <div className="report-page-body">
        <SectionHeading number="01" title={SECTION_01_TITLE} subtitle={SECTION_01_SUB} />
        <div className="report-card report-drivers-card">
          {DRIVER_DISPLAY_ORDER.map((key) => {
            ...
            return (
              <div key={key} className="report-driver-row">
        ...
        <div className="report-callout">
          <strong>
            {
              DRIVER_DISPLAY[
                DRIVER_DISPLAY_ORDER.reduce((best, key) =>
                  getDriverByKey(model, key).annualValue >
                  getDriverByKey(model, best).annualValue
                    ? key
                    : best,
                )
              ].headline
          ...
          is the largest modeled driver at {model.largestDriver.annualValueFormatted} annually
          in the moderate scenario.
        </div>
        <p className="report-note">{model.noDoubleCountingNotes[0]}</p>
      </div>
...
function LeaksPage({ model, logoSrc }: RoiReportProps) {
  ...
  return (
    <section className="report-page">
      <ReportHeader model={model} logoSrc={logoSrc} />
      <div className="report-page-body">
        <SectionHeading number="02" title={SECTION_02_TITLE} subtitle={SECTION_02_SUB} />
        ...
        <SectionHeading number="03" title={SECTION_03_TITLE} />
        <div className="report-leak-list">
          {(
            [
              ["Missed Calls", "missedCallRecovery"],
              ["No-Shows", "noShowReduction"],
              ["Upsell Revenue Left on the Table", "jobCloserUpsells"],
              ["A Customer List You're Not Using", "outboundSms"],
              ["Your Time", "timeSavings"],
            ] as const
          ).map(([titleMatch, driverKey]) => {
            const block = model.tradeContent.blocks.find((b) => b.title === titleMatch)!;
            const driver = getDriverByKey(model, driverKey);
            return (
              <div key={driverKey} className="report-leak-row">
                <div className="report-leak-name">{titleMatch}</div>
...
function MethodologyPage({ model, logoSrc }: RoiReportProps) {
  return (
    <section className="report-page">
      <ReportHeader model={model} logoSrc={logoSrc} />
      <div className="report-page-body">
        <SectionHeading number="04" title={SECTION_04_TITLE} />
        ...
        <p className="report-methodology">{METHODOLOGY_PARAGRAPH}</p>
        <ul className="report-trade-assumptions">
          {model.tradeAssumptions.map((line) => (
            <li key={line}>{line}</li>
          ))}
        </ul>
        <SectionHeading number="05" title={SECTION_05_TITLE} />
  ...
  return (
    <section className="report-page">
      ...
      <div className="report-page-body">
        ...
        <SectionHeading number="06" title="See it work on your calls" />
