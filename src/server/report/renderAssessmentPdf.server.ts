...
import { renderAssessmentHtml } from "~/server/report/renderAssessmentHtml.server";

export type AssessmentRenderTimingBreakdown = {
  mode: "cold" | "warm";
...
async function launchAssessmentBrowser(
...
export async function closeAssessmentBrowser(): Promise<void> {
  if (warmBrowser) {
...
export async function renderAssessmentPdf(
...
): Promise<{ pdf: Uint8Array; timing?: AssessmentRenderTimingBreakdown }> {
  const mode = options?.mode ?? "warm";
  const collectTiming = options?.collectTiming ?? false;
  const handlerStarted = performance.now();
  const timing: Partial<AssessmentRenderTimingBreakdown> = { mode };
  ...
  if (mode === "cold" || !browser) {
    ...
    browser = await launchAssessmentBrowser(puppeteer, chromium);
    if (mode === "warm") {
      warmBrowser = browser;
    }
  }
  ...
  const html = renderAssessmentHtml(model);
  ...
  return {
    pdf,
    timing: collectTiming ? (timing as AssessmentRenderTimingBreakdown) : undefined,
  };
}
