#!/usr/bin/env bun
/**
 * Scan the legacy Speed2Lead session store for ROI conversations still in flight.
 * Used before deleting the old orchestrator engine.
 */
...
function isBooked(context: AnyConversationContext): boolean {
  ...
  if (context.knownFacts?.discoveryPhase === "booked") return true;
  if (context.knownFacts?.peakDiscoveryPhase === "booked") return true;
  return false;
}
...
function isOldRoiEngineSession(context: AnyConversationContext): boolean {
  const flow = context.flow ?? context.knownFacts?.flow;
  return flow === "roi" || flow === undefined;
}
...
async function main() {
  ...
  const redis = getRedis();
  const active: ActiveOldSession[] = [];
  const now = Date.now();

  for (const key of keys) {
    const raw = await redis.get<AnyConversationContext>(key);
    if (!raw) continue;
    if (!isOldRoiEngineSession(raw)) continue;
    ...
    active.push({
      ...
      discoveryPhase: raw.knownFacts?.discoveryPhase,
      updatedAt,
    ...
    JSON.stringify(
      {
        ...
        activeOldEngineSessions: active.length,
        sessions: active,
      },
      null,
      2,
