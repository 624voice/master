#!/usr/bin/env bun
/**
 * Scan the legacy Speed2Lead session store for contact conversations still in flight.
 * Used before deleting src/server/contactSpeed2Lead/.
 */
...
function isOldContactEngineSession(context: AnyConversationContext): boolean {
  return context.flow === "contact";
}
...
async function main() {
  ...
  const redis = getRedis();
  const active: ActiveOldContactSession[] = [];
  const now = Date.now();

  for (const key of keys) {
    const raw = await redis.get<AnyConversationContext>(key);
    if (!raw) continue;
    if (!isOldContactEngineSession(raw)) continue;
    ...
    JSON.stringify(
      {
        ...
        activeOldContactEngineSessions: active.length,
        sessions: active,
      },
      null,
      2,
